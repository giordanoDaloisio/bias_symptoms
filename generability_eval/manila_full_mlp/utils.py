...
def cross_val(
    ...
    for train, test in fold.split(data_start):
        ...
        exp = bool(
            inprocessor == FairnessMethods.EG or inprocessor == FairnessMethods.GRID
        )
        ...
        compute_metrics(
            pred, unpriv_group, label, positive_label, metrics, sensitive_features
        )
...
def compute_metrics(
    df_pred, unpriv_group, label, positive_label, metrics, sensitive_features
):
